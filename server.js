...
const userUpload = multer({ storage: storageEngine(), limits: { fileSize: 10 * 1024 * 1024 } })
const adminUpload = multer({ storage: storageEngine(), limits: { fileSize: 25 * 1024 * 1024 } })
...
app.post("/threads/:id/reply", (req, res) => {
  userUpload.single("media")(req, res, err => {
    try {
      ...
      const user = ip(req)
      if (!canReply(user)) return res.status(429).json({ error: "slow down" })
...
app.get("/admin/threads", (req, res) => {
  ...
  res.json(
    threads.map(t => ({
      ...
      time: t.time,
      media: t.media,
      pendingCount: t.pendingReplies.length,
